...
pub fn generate_sim_file(file_path: &str) {
...
fn get_rid_of_characters(s: &str) -> &str{
    let mut index = 0;
    while s.chars().nth(index) == Some(' ') || s.chars().nth(index) == Some('\n'){
        index+=1;
    }
...
fn get_all_sections(s: &str) -> Result<Vec<Schema>, String> {
    let mut sec = section(get_rid_of_characters(s));
    match sec {
        Ok(mut sec_safe) => {
            ...
            while !sec_safe.1.is_empty() {
                ...
                if !sec_safe.0.is_empty() {
                    sec = section(get_rid_of_characters(sec_safe.0));
                    match sec {
                        Ok(temp) => {
                            sec_safe = temp;
                        }
...
mod tests {
    ...
    fn test_dectype() {
        ...
        assert_eq!(
            dectype("Protocol test='hello'"),
            Ok((" test='hello'", DecType::Protocol))
        );
        assert_eq!(
